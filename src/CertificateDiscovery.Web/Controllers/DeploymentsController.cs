...
public sealed class DeploymentsController(DeploymentService service) : Controller
{
    ...
    public IActionResult CreateTarget() => View(new DeploymentTargetUpsertRequest("Fake target", DeploymentTargetType.Fake, null, "{}", null, true));
    [HttpPost]
    public async Task<IActionResult> CreateTarget(DeploymentTargetUpsertRequest request, CancellationToken token) =>
        await RunFormAsync(() => service.CreateTargetAsync(request, token), nameof(CreateTarget), request);
    ...
    public IActionResult Create() => View(new DeploymentCreateRequest(Guid.Empty, Guid.Empty, Guid.Empty));
    [HttpPost]
    public async Task<IActionResult> Create(DeploymentCreateRequest request, CancellationToken token)
    {
        ...
        catch (Exception ex) { ModelState.AddModelError("", ex.Message); return View(request); }
    }
